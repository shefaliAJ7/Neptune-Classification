...
This file creates a Classification Model, use it to train on the Training Data 
and use it to predict the labels of Test data.

The classification model is a combination of Google Trained Word2vec + CNN + LSTM. 
It generates model for Binary Classification and Multi-Class Classification.

This File also provides K-Fold Cross Validation to test the biasness in the 
...
class Train(object):
    ...
    # train model K Times, plot accuracies 
    def train_model_with_kfold(self):
        ...
        plt.title('K Fold accuracy W2V PreTrained Multi Class - Struck-Sally-disagree')
        plt.ylabel('accuracy')
        plt.xlabel('Fold Value')
        plt.legend(['train', 'validation', 'test'], loc='lower right')
        plt.savefig("K Fold accuracy W2V PreTrained Multi Class - Struck-Sally-disagree.png", dpi=1200)
        plt.show()


    # train model for each k interation of K-Fold and calculate training, validation and test accuracy
    def train_model_for_every_k(self, x_train, y_train, x_test, y_test):
        model_file = MODEL_NAME
        model = Sequential()
        model.add(
            Embedding(self.vocabulary_size, 300, input_length=self.sentence_length, weights=[self.embedding_matrix],
                      trainable=True))
    ...
    # Generates model for SentenceClassifier - to get one label out of all labels (T, P, O, D, H) for one sentence
    ...
    # Generates model for MultiClassSentenceClassifier - to get multiple labels out of all labels (T, P, O, D, H) for one sentence
    ...
    # Generates model for BinarySentenceClassifier - to get one label out of two labels (U: useless, N: usefull) for one sentence
...
# This class is for Predicting the labels of Test Data and calculating Test Accuracy
